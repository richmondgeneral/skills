...
- **Phase 1 — Operating store (now).** Build the shared `item-model` core + Square/Whatnot adapters; land `rg-reconcile` (read/heal) and the write helper; put batch intake on the core; kill the dead category rule + the store-blocking rot; retire `catalog_index.jsonl` → `catalog_state.json`. Outcome: Commerce runs end-to-end, drift is detectable/healable, intake is reliable. **No monorepo migration required for this phase** — build the core/seam in place.
  ...
  - **Slice D — Reliable intake on the core (IN PROGRESS).** Phase 1's "intake is reliable" outcome is *partially met* — two halves:
    - **SKU allocation** — the RG-0030↔RG-0028 collision (2026-06-19) was a TOCTOU race on a stale-cache read, across **multi-machine** writers (Mac Claude Code, Cowork-over-bridge, cloud) that no filesystem lock can serialize. Solution already designed + approved: `rg-full-auto/docs/plans/2026-06-19-sku-allocation-design.md` — a Square catalog-object **version-CAS atomic counter** (`__RG_SKU_COUNTER__` sentinel) as the sole allocation authority (`allocate_sku()`/`bootstrap()`/`peek()` in a new `sku_authority.py`; the FS glob demoted to a bootstrap input). Slice D = write the impl plan + build it (bootstrap `N=30` → first mint RG-0031). *(Supersedes the RG-0030 postmortem's `.sku-lock` quick-fix — a filesystem lock can't serialize multi-machine writers.)*
    - **Intake-on-core + schema — DONE (on `feat/slice-d-intake-on-core`, pending merge).** Built: `item_model.measurements.compute_oversize` (any box dim > 24 in); the intake stub (`intake_to_item.stub_label`) now carries `eye_color`/`measurements_in`/`buyer_questions[]`/`oversize`; a `square_cache_sync` pricing preflight (`pricing_preflight.ensure_fresh_cache` — the stale $75→$45 misled RG-0030); a pure `resolve_staging_dir` (scratch stages outside `items/<sku>/`); and a **channel-status write-path** (`channel_registry.set_channel_status` + `rg-reconcile/scripts/set_channel_status.py`) recording a channel's status + platform IDs through the tested registry merge (sold-sticky, recomputes `oversize`) instead of hand-editing `label.json` — closing Slice B's deferred "transition channel statuses during intake." +18 unit tests, full suite green. *Follow-ups (deferred):* auto-wiring `process_batch.py` phases to call the write-path, the `--staging-dir`/`--promote` CLI + `items/.gitignore`, and aligning `set_channel_status`'s `archived` stickiness with `sold_from_label`. Sources: the 2026-06-19 RG-0030, RG-0028/0029, RG-0009 postmortems under `ops/reports/` (RG-0009 misfiled-then-consolidated into `ops/reports/`).
