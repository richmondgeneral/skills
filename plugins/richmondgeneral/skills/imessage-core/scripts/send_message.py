...
Usage: 
  ...
  python3 send_message.py --service <iMessage|SMS|RCS> <phone> "message"
"""
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  python3 send_message.py --service <iMessage|SMS|RCS> <phone> \"message\"")
        sys.exit(1)
    ...
    if sys.argv[1] == "--service":
        if len(sys.argv) < 5:
            print("Error: Need service type, phone, and message")
            print("Example: --service RCS +18473383408 \"Hello\"")
            sys.exit(1)
        svc_type = sys.argv[2]
        if svc_type not in SERVICE_IDS:
            print(f"Error: Unknown service '{svc_type}'. Use: iMessage, SMS, or RCS")
            sys.exit(1)
        phone = sys.argv[3]
        message = sys.argv[4]
        print(f"Forcing service: {svc_type}")
        success, result = send_to_contact(phone, message, SERVICE_IDS[svc_type])
        print(result)
