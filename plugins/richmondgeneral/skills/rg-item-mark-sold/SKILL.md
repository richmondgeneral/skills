...
metadata:
  ...
  changelog: |
    ...
    - --audit now resolves a link it can't attribute by record/metadata via the
      ...
      variation/object id, or a normalized line-item name -> the item's product
      name. A sold pre-ledger item with a null-metadata link (RG-0003: sold off
      Square, no recorded buy_link) now classifies CRITICAL, not WARN.
    - Orders are fetched (read-only batch-retrieve, chunked) ONLY for links left
      unresolved after the cheap record/metadata passes — minimal API + fuzzy
      matching. New helpers: batch_retrieve_orders(), order_signals(),
      _normalize_text(); attribute_link() gains order-catalog / order-name.
    - CRITICAL report line now shows how the link was matched.
    - Tests: order_signals extraction + order-name/order-catalog CRITICAL +
      still-WARN-when-no-match (tests/test_delete_payment_link.py).
