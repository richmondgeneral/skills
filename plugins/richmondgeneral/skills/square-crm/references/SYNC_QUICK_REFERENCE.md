...
python3 ${CLAUDE_PLUGIN_ROOT}/skills/square-crm/scripts/sync_to_apple_contacts.py +13129143889 --last-name "Giba"
```

### Scenario 2: Square shows alternate phone
```bash
# Add alt phone to their Apple Contact
python3 ${CLAUDE_PLUGIN_ROOT}/skills/square-crm/scripts/sync_to_apple_contacts.py +13129143889 --alt-phone "+17085970480"
```

### Scenario 3: Both name + alternate phone
```bash
# Update Apple Contacts with both
python3 ${CLAUDE_PLUGIN_ROOT}/skills/square-crm/scripts/sync_to_apple_contacts.py +13129143889 \
  --last-name "Giba" \
...
python3 ${CLAUDE_PLUGIN_ROOT}/skills/square-crm/scripts/sync_to_apple_contacts.py +14148757568 \
  --first-name "Bob" \
  --last-name "Mitchell"
...
# 3️⃣ Update Square
python3 ${CLAUDE_PLUGIN_ROOT}/skills/square-crm/scripts/parse_contacts.py --phone +14148757568
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/square-crm/scripts/sync_to_apple_contacts.py --json '{
  "phone": "+14148757568",
  "first_name": "Bob",
  "last_name": "Mitchell",
...
|------|---------|--------------|
| `${CLAUDE_PLUGIN_ROOT}/skills/contacts-manager/references/contacts.md` | Master contact reference | When discovering new customer info |
| `${CLAUDE_PLUGIN_ROOT}/skills/square-crm/SKILL.md` | Square skill docs | When new groups/APIs are available |
| `${CLAUDE_PLUGIN_ROOT}/skills/contacts-manager/SKILL.md` | iMessage skill docs | When adding new contact style notes |
...
grep "+14148757568" ${CLAUDE_PLUGIN_ROOT}/skills/contacts-manager/references/contacts.md

# 3. Square CRM Dashboard — search by phone
# 4. SKILL.md — check Quick Reference tables
grep "Bob Mitchell" ${CLAUDE_PLUGIN_ROOT}/skills/square-crm/SKILL.md
```
...
1. **Add to shell alias** for faster access:
   ```bash
   alias sync-contact="python3 ${CLAUDE_PLUGIN_ROOT}/skills/square-crm/scripts/sync_to_apple_contacts.py"
...
2. **Bulk upload after discovery:**
   ...
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/square-crm/scripts/parse_contacts.py
   ```

3. **Keep SKILL.md tables current:**
   - Update Quick Reference whenever someone's name changes
   - Run validation: `python3 ${CLAUDE_PLUGIN_ROOT}/skills/square-crm/scripts/parse_contacts.py --validate`
