...
- The issue is image **order** but each image is correct — just patch `item_data.image_ids` directly via `batchUpdateObjects` with `sparse_update:true` (send only `{type,id,version,item_data:{image_ids}}`, then re-verify the item's `variations`/`price`/`image_ids` in the response); no cache-bust needed
